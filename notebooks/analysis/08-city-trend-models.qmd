...
if (data_version == "standard") {
  input_suffixes <- c("", "_standard")
} else if (data_version %in% c("subunit0", "censored10", "raw")) {
  input_suffixes <- paste0("_", data_version)
} else {
  stop("data_version must be one of: 'standard', 'subunit0', 'censored10', 'raw'")
}

output_suffix <- paste0("_", data_version)

resolve_input_path <- function(paths) {
  existing <- paths[file.exists(paths)]
  if (length(existing) == 0) {
    stop(
      paste0(
        "Could not find any matching input file. Tried: ",
...
capital
# Read the CSV file
file_candidates <- paste0(
  wd, '/data/outputs/', country_short, '/mov-analysis/by-', category,
  '/trend_', place, input_suffixes, '.csv'
)
file_path <- resolve_input_path(file_candidates)
...
tidy_results_cat <- bind_rows(tidy_fixed, tidy_smooth) %>%
  distinct()
file_tidy_results_cat <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/data_trend_netflows_', place, '_param_tidy_results_cat', output_suffix, '.csv')
write.csv(tidy_results_cat, file_tidy_results_cat, row.names = FALSE)

glance_results_cat <- map_df(models_cat, glance, .id = "model")
file_glance_results_cat <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/data_trend_netflows_', place, '_param_glance_results_cat', output_suffix, '.csv')
...
file_tidy_results_cats <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/data_trend_netflows_', place, '_param_tidy_results_cats', output_suffix, '.csv')
write.csv(tidy_results_cats, file_tidy_results_cats, row.names = FALSE)

glance_results_cats <- map_df(models_cats, glance, .id = "model")
file_glance_results_cats <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/data_trend_netflows_', place, '_param_glance_results_cats', output_suffix, '.csv')
...
file_m3cat_re <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/data_trend_param_', place, '_m3cat_re', output_suffix, '.csv')
write.csv(m3cat_re[[1]], file_m3cat_re, row.names = FALSE)

file_m3cats_re <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/data_trend_param_', place, '_m3cats_re', output_suffix, '.csv')
write.csv(m3cats_re[[1]], file_m3cats_re, row.names = FALSE)

###

file_m4cat_re <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/data_trend_param_', place, '_m4cat_re', output_suffix, '.csv')
write.csv(m4cat_re[[1]], file_m4cat_re, row.names = FALSE)

file_m4cats_re <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/data_trend_param_', place, '_m4cats_re', output_suffix, '.csv')
write.csv(m4cats_re[[1]], file_m4cats_re, row.names = FALSE)

###

file_m5cat_re <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/data_trend_param_', place, '_m5cat_re', output_suffix, '.csv')
write.csv(m5cat_re[[1]], file_m5cat_re, row.names = FALSE)

file_m5cats_re <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/data_trend_param_', place, '_m5cats_re', output_suffix, '.csv')
write.csv(m5cats_re[[1]], file_m5cats_re, row.names = FALSE)

###

file_m6cat_re <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/data_trend_param_', place, '_m6cat_re', output_suffix, '.csv')
write.csv(m6cat_re[[1]], file_m6cat_re, row.names = FALSE)

file_m6cats_re <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/data_trend_param_', place, '_m6cats_re', output_suffix, '.csv')
write.csv(m6cats_re[[1]], file_m6cats_re, row.names = FALSE)

###

file_m7cat_re <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/data_trend_param_', place, '_m7cat_re', output_suffix, '.csv')
write.csv(m7cat_re[[1]], file_m7cat_re, row.names = FALSE)

file_m7cats_re <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/data_trend_param_', place, '_m7cats_re', output_suffix, '.csv')
...
```{r}
# Create predicted data frames for each available group in the random effects output
time_grid <- 1:300
re_data <- m6cat_re[[1]]

intercept_idx <- which(grepl("Intercept", re_data$term))
slope_idx <- which(grepl("^time$", re_data$term) | grepl("time\\)$", re_data$term))
n_curves <- min(length(intercept_idx), length(slope_idx))

pred_dfs <- map(seq_len(n_curves), function(i) {
  tibble(
    time = time_grid,
    time2 = time^2,
    fitted = (m6cat$fit$par[[1]] + re_data$estimate[[intercept_idx[i]]]) +
      (m6cat$fit$par[[2]] + re_data$estimate[[slope_idx[i]]]) * time +
      m6cat$fit$par[[3]] * time2,
    group = re_data$group[[intercept_idx[i]]]
  )
})
...
if (stringency == FALSE) {
  if (capital == TRUE) {
    file_results_alpha <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/recovery-times-capital-gam', output_suffix, '-r2.csv')
  } else {
    file_results_alpha <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/recovery-times-fuas-gam', output_suffix, '-r2.csv')
  }
} else {
  if (capital == TRUE) {
    file_results_alpha <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/recovery-times-capital-gams', output_suffix, '-r2.csv')
  } else {
    file_results_alpha <- paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/recovery-times-fuas-gams', output_suffix, '-r2.csv')
  }
}
