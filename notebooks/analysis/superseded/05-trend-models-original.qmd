...
```{r}
category <- "density"
data_version <- "new"  # use "subunit0" for the conservative-imputed branch
output_version <- ifelse(data_version == "new", "new", data_version)
```

```{r}
# Read the CSV file
file_path <- if (data_version == "new") {
  paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/data_trend_new.csv')
} else {
  paste0(wd, '/data/outputs/', country_short, '/mov-analysis/by-', category, '/data_trend_', data_version, '.csv')
}
...
df_old <- read.csv(paste0(wd, '/data/inputs/population/worldpop/old-age-dependency/', country_short, '/per_old_cat_', category, '.csv')) %>% rename("O" = "X")

df <- df %>% left_join(df_stringency, by = "time") %>% rename("O" = "cat") %>% left_join(df_old, by = "O")
```
